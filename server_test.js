...
module.exports = testCase({
  ...
  testSendWithoutId : function(test){
  ...
  testJoinSendAndReceive : function(test){
  ...
  testSubmitBadYoutubeLink : function(test){
  ...
  testSubmitGoodYoutubeLink : function(test){
    server.channel.messages = [] 
    var request = client.request('POST', '/submit_youtube_link', {"content-type":'urlencoded'}).on('response', function (response) {
      test.equal(200, response.statusCode);
      test.equal(server.channel.messages[0].title, 'He\'s Got The Whole World/One Light, One Sun/This Little Light Of Mine');
      test.done();
    })
  ...
  testSubmitSoundcloudLink : function(test){
    //server.channel.messages = [] 
  ...
  testSubmitNonFile : function(test){
    server.channel.messages = [] 
  ...
  testUploadMultipleFiles : function(test){
    server.channel.messages = []
    fs.unlink("tmp/thefile.mp3")
    fs.unlink("tags/thefile.mp3")
    fs.stat("boogoo")
